...
returns the length of the string for the radix n numeral denoting the
the value m

Note, we are looking for the smallest value k such thatthe sum, for i
running from 0 to k, of n raised to the power i, is GREATER THAN m.
...
The iteration is for the sum described in the assignment, but one
needs to test for overflow while doing it.
...
if (multWouldOverflow)
   /*
     ...
     currSumOfPowers is the latter sum of the two

   */
   if (currSumOfPowers <= m) //  m <= MAX < sum for i from 0 to the (currExpOfPower + 1)
      return currExpOfPower + 1;
   else
      // sum for i from 0 to (currExpOfPower - 1) <= m < currSumOfPowers = sum for i from 0 to currExpOfPower
   ...
   if (sumWouldOverflow)
      ...
      */
      ...
      /*
         ...
         1 <=  m < currentSumOfPowers = sum for i from 0 to currExpOfPower - 1
